...
(function loadTextures(){
  ...
  // Fix #7: esperar TODOS os frames antes de marcar waterReady
    ...
    for(let tx=Math.max(0,stx);tx<etx;tx++){
      ...
      if(t===4){ // Árvore
        if(TEX.treesReady) {
            ...
            if (tImg) {
                // Draw tree spanning 2 tiles tall (64px), centered, bottom-anchored at tile base
                const treeH = TILE * 2; // 64px — 2 tiles tall
                const treeW = treeH * (tImg.width / tImg.height);
                const drawX = sx + (TILE - treeW) / 2;
                const drawY = sy + TILE - treeH; // anchors at bottom of this tile, extends up
                ctx.drawImage(tImg, drawX, drawY, treeW, treeH);
            }
        } else {
...
function tick(now){
  ...
  ctx.clearRect(0, 0, VIEW_W, VIEW_H); // Fix #6: clearRect em vez de reatribuir canvas.width (evita reflow)
  const{cx,cy}=camPos();
...
// Fix #15: expor stopWorld para parar o RAF a partir de outros módulos
function stopWorld(){
...
function buildHeroes(){
  ...
  HEROES.forEach(h=>{
    ...
    btn.addEventListener('click',()=>{
      ...
      // Update name input with hero default if empty or was previous hero name
...
function drawPreview(h){
  ...
  // Hero name overlay (shows over the gradient at bottom)
...
function showScreen(id){
  ...
  // Show mini menu on game and battle screens
  ...
  // Close dropdown when switching screens
...
document.addEventListener('DOMContentLoaded',()=>{
  // Auth handles first screen. auth.js initialises before game.js DOMContentLoaded runs,
  // but we guard here: if no session, go to authScreen; otherwise homeScreen will be shown by AuthUI.
  ...
  document.getElementById('menuHomeBtn').addEventListener('click', () => {
    if (confirm('Tens a certeza que queres voltar ao início?\nO progresso actual será perdido.')) {
      ...
      // Fix #8: limpar estado e parar RAF antes de sair
      S.inBattle  = false;
      S.enemy     = null;
      S.curQ      = null;
      S.qCache    = [];
...
// ── API KEY UI INJECTION ──────────────────────────────────────
function _injectApiKeyUI(){
  // Fix #1: não sair cedo — inicializar sempre os listeners mesmo que o elemento já exista no HTML estático
  const existingInput = document.getElementById('apiKeyInput');

  if (!existingInput) {
    // Injetar o HTML dinamicamente apenas se não estiver no DOM
    ...
    const fileDiv = fileZone.parentElement;
    parent.insertBefore(section, fileDiv);
  }

  // Inicializar sempre (quer tenha sido injetado agora quer já existisse)
  const ki=document.getElementById('apiKeyInput');
  if(!ki) return;

  // Guardar contra dupla inicialização
  if(ki.dataset.apiInitialized) return;
  ki.dataset.apiInitialized='true';

  // Carregar chave guardada
  ...
  // Atualizar descrição da zona de ficheiro
  const fzDesc=document.querySelector('#fileZone p');
  if(fzDesc) fzDesc.textContent='Clica ou arrasta o teu ficheiro .txt ou .pdf aqui';
}
...
function _setApiStatus(status){
  ...
  if(!el) return;
  if(status==='valid')   { el.textContent='✅ Formato de chave válido'; el.className='api-key-status status-ok'; }
  else if(status==='invalid'){ el.textContent='⚠️ A chave deve começar com sk-ant-'; el.className='api-key-status status-warn'; }
  else if(status==='saved')  { el.textContent='💾 Chave carregada do armazenamento local'; el.className='api-key-status status-ok'; }
