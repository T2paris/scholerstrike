...
// ── TEXTURAS ────────────────────────────────────────────────
const TEX = { grassImg: null, grassReady: false, pathImg: null, pathReady: false, treeTiles: [], treesReady: false, heroes: {} };
...
function walkable(px, py) {
  ...
  const check=(x,y)=>{
    ...
    const t=map[y*mw+x];
    return t!==1&&t!==2&&t!==4&&t!==7&&t!==8&&t!==9;
  };
  const tx=Math.floor(px/TILE), ty=Math.floor(py/TILE);
  const tx2=Math.floor((px+TILE-2)/TILE), ty2=Math.floor((py+TILE-2)/TILE);
  return check(tx,ty)&&check(tx2,ty)&&check(tx,ty2)&&check(tx2,ty2);
}
...
function drawMap(cx,cy){
  ...
  for(let ty=Math.max(0,sty);ty<ety;ty++){
    for(let tx=Math.max(0,stx);tx<etx;tx++){
      ...
      // Relva: usar textura real se disponível
      if(t===0 && TEX.grassReady){
      ...
      if(t===5){ // Ruína
        ...
        ctx.fillRect(sx+2,sy+2,TILE-4,5); ctx.fillRect(sx+2,sy+TILE-7,TILE-4,5);
        ctx.fillStyle='rgba(0,0,0,.22)';  ctx.fillRect(sx+8,sy+8,TILE-16,TILE-16);
      }
      if(t===6){ // Portal — brilha e pulsa
        const p=0.35+Math.sin(now/400)*0.2;
        ctx.fillStyle=`rgba(120,60,255,${p})`; ctx.fillRect(sx+4,sy+4,TILE-8,TILE-8);
        ctx.fillStyle=`rgba(200,150,255,${p*0.5})`; ctx.fillRect(sx+9,sy+9,TILE-18,TILE-18);
        ctx.strokeStyle=`rgba(220,180,255,${p+0.1})`; ctx.lineWidth=1;
        ctx.strokeRect(sx+2,sy+2,TILE-4,TILE-4);
      }
      if(t===7){ // Edifício
        ctx.fillStyle='rgba(255,255,255,.06)'; ctx.fillRect(sx+2,sy+2,TILE-4,7);
        ctx.fillStyle='rgba(0,0,0,.45)';
        ctx.fillRect(sx+7,sy+10,8,10); ctx.fillRect(sx+19,sy+10,8,10);
        ctx.fillStyle='rgba(255,200,100,.18)';
        ctx.fillRect(sx+8,sy+11,6,8);  ctx.fillRect(sx+20,sy+11,6,8);
      }
      if(t===8){ // Dungeon
        ctx.fillStyle='rgba(0,0,0,.55)'; ctx.fillRect(sx+4,sy+4,TILE-8,TILE-8);
        ctx.fillStyle='rgba(100,50,150,.35)';
        ctx.beginPath(); ctx.arc(sx+16,sy+16,7,0,Math.PI*2); ctx.fill();
      }
      if(t===9){ // Lava
        const l=now/600;
        ctx.fillStyle=`rgba(255,80,0,${0.28+Math.sin(l+tx)*0.12})`;
        ctx.fillRect(sx,sy,TILE,TILE);
        ctx.fillStyle=`rgba(255,180,0,${0.18+Math.cos(l+ty)*0.08})`;
...
function drawPlayer(cx,cy){
  ...
  if (S.hero && TEX.heroes[S.hero.img]) {
      ctx.drawImage(TEX.heroes[S.hero.img], sx - 4, sy - 8, 40, 40);
  } else {
...
function tick(now){
  ...
  const{cx,cy}=camPos();
  drawMap(cx,cy); drawNpcs(cx,cy); drawPlayer(cx,cy);
  ...
  if(!S.inBattle&&!S.dialogNpc&&S.cooldown<=0&&inEncZone(S.px,S.py)&&Math.random()<getEncRate()){
